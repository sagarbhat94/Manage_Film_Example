...
public class FilmBaseService {
	...
	public Film insert(Film obj) {
		Long id = jdbcTemplate.queryForObject("select max(_id) from `film`", new MapSqlParameterSource(), Long.class);
		obj.set_id(id == null ? 1 : id + 1);
		String sql = "INSERT INTO `film` (`_id`, `genre`,`title`,`year`,`filmMaker`) VALUES (:id,:genre,:title,:year, :filmMaker )";
	...
	public void delete(Long id) {
		String sql = "DELETE FROM `Film` WHERE `_id`=:id";
		SqlParameterSource parameters = new MapSqlParameterSource()
			.addValue("id", id);
		
		String sql_cast = "DELETE FROM `film_cast` WHERE `id_Film`= :id";
		jdbcTemplate.update(sql_cast, parameters);
		jdbcTemplate.update(sql, parameters);
	}
	...
	public List<Film> findBycast(Long idcast) {
        ...
        String sql = "select * from `Film` WHERE `_id` IN (SELECT `id_Film` FROM `Film_cast` WHERE `id_Actor` = :idcast)";
		
	    SqlParameterSource parameters = new MapSqlParameterSource()
		.addValue("idcast", idcast);
	    
	    return jdbcTemplate.query(sql, parameters, new BeanPropertyRowMapper(Film.class));
	}
	...
	public Film get(Long id) {
		...
		String sql = "select * from `Film` where `_id` = :id";
		
	    SqlParameterSource parameters = new MapSqlParameterSource()
			.addValue("id", id);
	    
	    return (Film) jdbcTemplate.queryForObject(sql, parameters, new BeanPropertyRowMapper(Film.class));
	}
	...
	public Film update(Film obj, Long id) {
		...
		String sql = "UPDATE `Film` SET `genre` = :genre,`title` = :title,`year` = :year , `filmMaker` = :filmMaker  WHERE `_id`=:id";
		SqlParameterSource parameters = new MapSqlParameterSource()
			.addValue("id", id)
			.addValue("genre", obj.getGenre())
			.addValue("title", obj.getTitle())
    ...
    public static class Film_castService {
    	...
    	public static void insert(Long id_Film, Long id_Actor) {
			...
			Long id = jdbcTemplate.queryForObject("select max(_id) FROM Film_cast", new MapSqlParameterSource(), Long.class);
			obj.set_id(id == null ? 1 : id + 1);
			
			String sql = "INSERT INTO Film_cast (`_id`, `id_Film`, `id_Actor` )	VALUES (:id, :id_Film, :id_Actor)";
